...
# =====================================================================
# GET ENDPOINT: FETCH AND GROUP
# =====================================================================
@router.get("/", response_model=List[DepartmentGroupResponse])
def get_department_rules_grouped():
    try:
        # 1. Fetch Flat Data from DB
        raw_records = DepartmentRules.get_all_rules() # Get All rules (Model File)

        # 2. Brainstorming Logic: Transformation
        # We need to map: 
        # [ {dept: A, rule: 1}, {dept: A, rule: 2}, {dept: B, rule: 3} ]
        # INTO -> 
        # [ {dept: A, rules: [1, 2]}, {dept: B, rules: [3]} ]
        ...
        for record in raw_records:
            ...
            # 3. Resolve the Value (The Merge Logic)
            # If type is number, take threshold. If boolean, take boolean_value.
            final_value = None
            ...
            # 4. Create the Rule Object
            rule_obj = {
                "id": record.id,
                "rule_key": record.rule_key,
                "rule_name": record.rule_name,
                "rule_type": record.rule_type,
                "value": final_value, # Normalized value
                "enabled": record.enabled
            }

            # Add to the specific department's list
            grouped_data[d_key]["rules"].append(rule_obj)

        # 5. Convert Dictionary values to List for API Response
