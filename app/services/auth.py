...
def build_user_response(user: User) -> dict:
    ...
    return {
        ...
        "allowed_actions": getattr(user, "allowed_actions", []) or [],
...
async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    user_check = UserService.get_user_by_username(
        form_data.username
    ) or UserService.get_user_by_email(form_data.username)
    ...
    if not user:
        ...
        if user_check:
            ...
            elif not UserService.verify_password(
                form_data.password, user_check.password
            ):
        ...
        error_detail = (
            "Incorrect username or password"  # Default for invalid credentials
        )
    ...
    with DBSession(engine) as db_session:
        user_session = UserSession(
            ...
            + timedelta(hours=24),  # 24 hour session
            is_active=True,
...
async def forgot_password(request: Request, email: str):
    ...
    AuditService.log_business_event(
        ...
        resource="auth",
        resource_id=str(user.id),
        new_values={"email": email, "reset_token_generated": True},
        ip_address=request.client.host,
        user_agent=request.headers.get("User-Agent"),
        status="success",
    )
...
async def check_token_status(token: str = Depends(oauth2_scheme)):
    ...
    if not jti:
        return {"status": "invalid", "message": "Token does not have JWT ID"}
