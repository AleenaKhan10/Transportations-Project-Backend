...
class UserService:
    ...
    def create_pending_verification(user_data: UserCreate) -> PendingEmailVerification:
        ...
        with Session(engine) as session:
            ...
            if existing:
                ...
                existing.expires_at = utc_now() + timedelta(
                    minutes=10
                )  # 10 minutes expiry
    ...
    @staticmethod
    def update_user(
        user_id: int, user_data: UserUpdate, updated_by: Optional[int] = None
    ) -> Optional[User]:
        with Session(engine) as session:
...
class SessionService:
    @staticmethod
    def create_session(
        user_id: int, ip_address: str, user_agent: str, expires_in_hours: int = 24
    ) -> UserSession:
        with Session(engine) as session:
